...
import avatar from '../../src/assets/images/doc.png'
...
const Profile = ({user}) => {
  ...
  const handleFileInputChange = async (event) => {
    ...
    // Update formData.photo with the URL from Cloudinary
    ...
    <div className="mt-10">
      <form onSubmit={submitHandler}>
          ...
          <div className="mb-5">
            ...
            className='w-full pr-4 py-3 border-b border-solid border-[#0066ff61] focus:outline-none
            focus:border-b border-primaryColor text-[16px] leading-7 text-headingColor placeholder:text-textColor rounded-md cursor-pointer'
            />
          </div>
          

          <div className="mb-5">
            <input type='text' placeholder='Enter Blood type' name='bloodType' value={formData.name} onChange={handleInputChange}
            className='w-full pr-4 py-3 border-b border-solid border-[#0066ff61] focus:outline-none
            focus:border-b border-primaryColor text-[16px] leading-7 text-headingColor placeholder:text-textColor  cursor-pointer' required
            />
          </div>
          ...
          <div className="mt-7">
            <button type='submit' className='w-full bg-primaryColor text-white text-[20px] leading-[30px] rounded-lg px-4 py-3'>
              Update
            </button>
          </div>
